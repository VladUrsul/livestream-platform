...
func NewServiceProxy(targetURL string) (*ServiceProxy, error) {
	...
	// NewSingleHostReverseProxy already has a proper Director function
...
func (p *ServiceProxy) Forward() gin.HandlerFunc {
	return func(c *gin.Context) {
		fmt.Printf("[PROXY] Forwarding %s %s to %s\n", c.Request.Method, c.Request.URL.Path, p.target.Host)
		// Capture response before it's sent to client
		statusBefore := c.Writer.Status()
		// Just call ServeHTTP - NewSingleHostReverseProxy handles URL rewriting via Director
		p.proxy.ServeHTTP(c.Writer, c.Request)
		statusAfter := c.Writer.Status()
		fmt.Printf("[PROXY] Response status: %d (before: %d)\n", statusAfter, statusBefore)
	}
}
